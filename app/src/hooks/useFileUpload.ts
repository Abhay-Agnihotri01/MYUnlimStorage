...
export function useFileUpload(activeFolderId: number | null, store: AppStore | null, targetLabel = 'Saved Messages') {
    ...
    // Wake Lock API for Mobile browsers: Prevent screen from sleeping during active uploads
    ...
    useEffect(() => {
        ...
        const acquireWakeLock = async () => {
            ...
            try {
                if ('wakeLock' in navigator) {
                    ...
                    console.log('Wake Lock acquired to prevent screen sleep during upload.');
                }
            } catch (err) {
        ...
        // Handle page visibility changes (Android might release the lock when backgrounded)
        ...
        return () => {
            ...
            // Don't release on cleanup unless component unmounts entirely
        };
    }, [uploadQueue]);
