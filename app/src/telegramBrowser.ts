...
import { analyzeImageWithGemini } from './aiService';
...
type DriveManifestSettings = {
    ...
    autoIndexLegacyMessages: boolean;
    geminiApiKey?: string;
    geminiModel?: string;
};
...
export async function telegramFactoryReset(): Promise<boolean> {
    ...
    manifestCache = empty;
    persistManifestLocally(empty);
    await writeRemoteManifest(empty);
    return true;
}
...
export async function telegramGetFiles(folderId: number | null | undefined = null, query?: string): Promise<TelegramFile[]> {
    ...
    for (const message of messages) {
        ...
        if (!wasIndexed && manifest.settings.autoIndexLegacyMessages === false) continue;
...
export async function telegramUploadFile(
    ...
    let aiTags: string[] = [];
    if (file.type.startsWith('image/') && manifest.settings.geminiApiKey) {
        aiTags = await analyzeImageWithGemini(manifest.settings.geminiApiKey, file, manifest.settings.geminiModel);
    }
...
export async function telegramUpdateManifestSettings(settingsUpdate: Partial<DriveManifestSettings>): Promise<void> {
    const manifest = await getDriveManifest();
    manifest.settings = { ...manifest.settings, ...settingsUpdate };
    await saveDriveManifest(manifest, 'immediate');
}
...
export async function telegramSetAutoIndexLegacy(enabled: boolean): Promise<boolean> {
    const manifest = await getDriveManifest();
    manifest.settings.autoIndexLegacyMessages = enabled;
    await saveDriveManifest(manifest, 'debounced');
    return true;
}

export async function telegramPurgeLegacyMessages(): Promise<{ deleted: number }> {
    const manifest = await getDriveManifest();
    let deleted = 0;
    for (const [key, record] of Object.entries(manifest.files)) {
        if (!record.checksum && (!record.tags || record.tags.length === 0) && !record.versionGroup && !record.originalPath) {
            delete manifest.files[key];
            delete manifest.fileFolders[key];
            deleted++;
        }
...
export async function telegramRepairManifest(): Promise<{
    ...
    for (const message of messages) {
        ...
        const wasIndexed = Boolean(manifest.files[key]);
        if (!wasIndexed && manifest.settings.autoIndexLegacyMessages === false) continue;
...
function normalizeManifest(manifest: Partial<DriveManifest>): DriveManifest {
    ...
    return {
        ...
        settings: {
            ...
            duplicateStrategy: 'version',
            autoIndexLegacyMessages: manifest.settings?.autoIndexLegacyMessages ?? false,
            geminiApiKey: manifest.settings?.geminiApiKey,
            geminiModel: manifest.settings?.geminiModel,
        },
    };
}
...
function buildDriveStats(manifest: DriveManifest): DriveStats {
    ...
    return {
        ...
        autoIndexLegacyMessages: manifest.settings.autoIndexLegacyMessages,
        geminiApiKey: manifest.settings.geminiApiKey,
        geminiModel: manifest.settings.geminiModel,
        largestFiles: active
            .slice()
...
export function telegramGetBandwidth() {
    try {
        const raw = localStorage.getItem(scopedKey(BANDWIDTH_KEY)) || localStorage.getItem(BANDWIDTH_KEY);
        if (raw) {
            const parsed = JSON.parse(raw);
            return {
                up_bytes: Number(parsed.up_bytes) || 0,
                down_bytes: Number(parsed.down_bytes) || 0,
            };
        }
    } catch {
