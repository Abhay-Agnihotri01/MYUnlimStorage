...
import { Check, Folder, Lock, MoreVertical, Shield, Trash2 } from 'lucide-react';
            ...
            >
                <div className="relative mx-2 mt-2 aspect-[1.78] overflow-hidden rounded-md border border-telegram-border bg-telegram-hover/45">
                    <button
                        ...
                        className={`absolute left-2 top-2 z-10 flex h-6 w-6 items-center justify-center rounded-full border transition ${isSelected ? 'border-telegram-primary bg-telegram-primary text-black opacity-100' : 'border-white/60 bg-black/25 text-transparent hover:text-white opacity-40 group-hover:opacity-100'} ${showSelectionControl && !isSelected ? 'opacity-100' : ''}`}
                        aria-label={isSelected ? 'Unselect item' : 'Select item'}
                    >
                        <Check className="h-3.5 w-3.5" />
                    </button>
                    {!showSelectionControl && !isSelected && onDelete && (
                        <button
                            type="button"
                            onClick={(event) => {
                                event.stopPropagation();
                            ...
                            className="absolute right-2 top-2 z-10 flex h-6 w-6 items-center justify-center rounded-full bg-black/40 text-white opacity-40 transition hover:bg-red-500 hover:opacity-100 group-hover:opacity-100"
                            aria-label="Delete"
                            title="Delete"
                        >
