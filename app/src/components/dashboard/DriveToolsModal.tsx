...
export function DriveToolsModal({
    ...
    const [autoIndexLegacy, setAutoIndexLegacy] = useState(false);
    ...
    const refresh = async () => {
        ...
        setAutoIndexLegacy(nextStats.autoIndexLegacyMessages ?? false);
    };
    ...
    const toggleAutoIndexLegacy = () => runBusy('autoIndex', async () => {
        const nextValue = !autoIndexLegacy;
        await invokeCommand('cmd_set_auto_index_legacy', { enabled: nextValue });
        toast.success(nextValue ? 'Legacy messages will now be indexed.' : 'Legacy messages will be hidden.');
    });

    const purgeLegacyMessages = () => runBusy('purgeLegacy', async () => {
        if (!window.confirm('This will completely clear all legacy messages from your index. Continue?')) return;
        const result = await invokeCommand<{ deleted: number }>('cmd_purge_legacy_messages');
        toast.success(`Purged ${result.deleted} legacy message(s) from your index.`);
    });
        ...
        <div className="fixed inset-0 z-[210] flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm" onClick={onClose}>
            <div className="flex max-h-[90vh] w-full max-w-5xl flex-col overflow-hidden rounded-xl border border-telegram-border bg-telegram-surface shadow-2xl" onClick={(e) => e.stopPropagation()}>
                ...
                <div className="custom-scrollbar grid gap-4 overflow-auto p-4 lg:grid-cols-2">
                    ...
                    <section className="rounded-lg border border-telegram-border p-4">
                        <Header icon={<Database className="h-4 w-4" />} title="Indexing Preferences" />
                        <div className="mt-3 space-y-3 text-sm text-telegram-subtext">
                            <p>Automatically scan and show files that were sent to Saved Messages outside of Telegram-Drive.</p>
                            <button onClick={toggleAutoIndexLegacy} disabled={busy === 'autoIndex'} className={autoIndexLegacy ? 'tool-btn' : 'tool-btn-danger'}>
                                {autoIndexLegacy ? 'Auto-indexing Enabled' : 'Auto-indexing Disabled (Hidden)'}
                            </button>
                            <button onClick={purgeLegacyMessages} disabled={busy === 'purgeLegacy'} className="tool-btn-danger">
                                Purge Indexed Legacy Messages
                            </button>
                        </div>
                    </section>
