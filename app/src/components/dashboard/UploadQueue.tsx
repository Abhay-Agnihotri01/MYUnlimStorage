                ...
                {items.map(item => (
                    <div key={item.id} className="flex flex-col gap-1 p-2 bg-telegram-hover rounded">
                        <div className="flex items-center gap-3 text-sm">
                            ...
                            <div 
                                className={`${getUploadStatusTextClass(item)} ${item.status === 'success' && onRemoveItem ? 'cursor-pointer hover:opacity-80' : ''}`}
                                onClick={() => {
                                    if (item.status === 'success' && onRemoveItem) onRemoveItem(item.id);
                                }}
                                title={item.status === 'success' ? 'Click to dismiss' : undefined}
                            >
