...
import { TelegramFile, BandwidthStats, TelegramFolder, DriveView, DriveStats } from '../types';
...
export function Dashboard({ onLogout }: { onLogout: () => void }) {
    ...
    useEffect(() => {
        ...
        let newView: DriveView = 'files';
        if (path.startsWith('/trash')) newView = 'trash';
        else if (path.startsWith('/collections')) newView = 'collections';
        else if (path.startsWith('/timeline')) newView = 'timeline';
        ...
        const folderMatch = path.match(/^\/drive\/folder\/(\d+)/);
        if (newView === 'files') {
            if (folderMatch) {
    ...
    const setDriveView = useCallback((view: DriveView) => {
        if (view === 'files') navigate('/drive');
    ...
    const setShowSettings = useCallback((show: boolean) => {
        if (show) navigate('/settings');
        else navigate(driveView === 'files' ? (activeFolderId ? `/drive/folder/${activeFolderId}` : '/drive') : `/${driveView}`);
    }, [navigate, driveView, activeFolderId]);
...
const EMPTY_FILES_ARRAY: TelegramFile[] = [];
    ...
    const { data: queryData, isLoading, error } = useQuery({
        ...
        queryFn: () => {
            const command = driveView === 'trash'
                ? 'cmd_get_trash_files'
                : driveView === 'collections' || driveView === 'timeline'
                    ? 'cmd_search_global'
                    : 'cmd_get_files';
    ...
    const allFiles = queryData || EMPTY_FILES_ARRAY;
    ...
    const uploadTargetLabel = useMemo(() => (
        driveView === 'files' ? getReadableFolderPath(activeFolderId, folders) : 'Saved Messages'
    ), [activeFolderId, driveView, folders]);
    ...
    const handleSyncAndStamp = useCallback(async () => {
    ...
    const syncStatusText = useMemo(() => {
        ...
        return 'Ready';
    }, [isSyncing, lastSyncAt, uploadQueue]);
    ...
    const restoreItems = useCallback(async (items: TelegramFile[]) => {
        ...
        if (missingOriginalParents.length > 0) {
            const ok = await confirm({
                title: "Restore to Saved Messages",
                message: `${missingOriginalParents.length} item(s) have a missing original folder. Restore them to Saved Messages instead?`,
                confirmText: "Restore",
                variant: 'info',
            });
            if (!ok) return;
        ...
        if (failed > 0) {
            const message = friendlyDriveError(lastError);
            toast.error(`Failed to restore ${failed} item(s): ${message}`, savedMessagesDefault ? {
                action: {
                    label: 'Repair Index',
                    onClick: () => {
                        void invokeCommand('cmd_repair_manifest')
                            .then(handleSyncAndStamp)
                            .then(() => toast.success('Index repaired. Try restore again.'))
                            .catch((err) => toast.error(`Repair failed: ${friendlyDriveError(err)}`));
                    },
                },
            } : undefined);
        }
    }, [confirm, folders, handleSyncAndStamp, queryClient, savedMessagesDefault]);
    ...
    }, [activeFolderId, confirm, displayedFiles, driveView, ensureProtectedAccess, handleBulkDelete, handleSyncAndStamp, queryClient, restoreItems, savedMessagesDefault, selectedIds]);
        ...
        const timer = setTimeout(async () => {
            ...
            if (driveView === 'trash') {
                ...
                const folderResults = folders
                    .filter((folder) => !folder.trashed && folder.name.toLowerCase().includes(searchTerm.toLowerCase()))
                    .map(folderToExplorerItem);
    ...
    const handleFileClick = async (e: React.MouseEvent, id: number) => {
        ...
        if (clicked?.type === 'folder') {
            ...
            if (searchTerm.length > 2 && driveView === 'files') {
                handleOpenFolderId(clicked?.folderId ?? null);
                setSearchTerm("");
                setSearchResults([]);
                window.setTimeout(() => {
                    setHighlightedId(id);
    ...
    }, [ensureProtectedAccess, folders, navigate]);
    ...
    const handleManualFolderUpload = useCallback(async () => {
        ...
        const uploadEntries: { file: File; folderId: number | null; targetLabel?: string }[] = [];
        ...
        for (const file of files) {
            ...
            uploadEntries.push({ file, folderId: targetFolder.id, targetLabel: getReadableFolderPath(targetFolder.id, knownFolders) });
        }

        handleOpenFolderId(rootFolder.id);
        setDriveView('files');
    ...
    }, [handleSyncAndStamp, queryClient, savedMessagesDefault]);
    ...
    const handleSelectedBulkAiAnalyze = useCallback(async () => {
        if (!store) return;
        const stats = await invokeCommand<DriveStats>('cmd_get_drive_stats', {});
        const apiKey = stats.geminiApiKey;
        const customModel = stats.geminiModel;
        if (!apiKey) {
            toast.error("Please configure your Gemini API Key in Settings first.");
            return;
        }

        const targets = displayedFileItems.filter((file) => selectedIds.includes(file.id) && file.mime_type?.startsWith('image/'));
        if (targets.length === 0) {
            toast.info("Select at least one image file to analyze.");
            return;
        }

        const { getBrowserBlob } = await import('../platform');
        const { analyzeImageWithGemini } = await import('../aiService');
        
        let success = 0;
        let errors = 0;
        
        toast.info(`Analyzing ${targets.length} images...`);
        for (const file of targets) {
            try {
                const { blob } = await getBrowserBlob(file.id);
                const tags = await analyzeImageWithGemini(apiKey, blob, customModel);
                ...
                console.error('AI Analyze error on file', file.id, e);
                errors++;
                if (errors >= 3) {
                    toast.error("Too many AI errors (likely Quota Exceeded). Stopping bulk analysis.");
                    break;
                }
            }
        }
        ...
        queryClient.invalidateQueries({ queryKey: ['files'] });
        if (success > 0) toast.success(`AI successfully tagged ${success} image(s).`);
        if (errors > 0) toast.error(`Failed to analyze ${errors} image(s).`);
        setSelectedIds([]);
    }, [displayedFileItems, queryClient, selectedIds, store]);

    const handleAnalyzeMissingAi = useCallback(async () => {
        if (!store) return;
        const stats = await invokeCommand<DriveStats>('cmd_get_drive_stats', {});
        const apiKey = stats.geminiApiKey;
        const customModel = stats.geminiModel;
        ...
        if (targets.length === 0) {
            toast.info("All images have already been AI analyzed!");
            return;
        }
        ...
        toast.info(`Analyzing ${targets.length} unanalyzed images...`);
        for (const file of targets) {
            try {
                const { blob } = await getBrowserBlob(file.id);
                const tags = await analyzeImageWithGemini(apiKey, blob, customModel);
                ...
                errors++;
                if (errors >= 3) {
                    toast.error("Too many AI errors (likely Quota Exceeded). Stopping analysis.");
                    break;
                }
            }
        }
    ...
    const handleCreateFolderHere = useCallback(async () => {
        ...
        if (!name) return;
        try {
            await handleCreateFolder(name, activeFolderId, true);
            queryClient.invalidateQueries({ queryKey: ['files'] });
            await handleSyncAndStamp();
            toast.success(`Created "${name}" in ${uploadTargetLabel}.`);
        } catch {
    ...
    }, [handleSyncAndStamp, queryClient]);
        ...
        >
            ...
            <main className="flex min-w-0 flex-1 flex-col" onClick={(e) => { if (e.target === e.currentTarget) setSelectedIds([]); }}>
                <TopBar
                    ...
                    onBulkAiAnalyze={handleSelectedBulkAiAnalyze}
                ...
                {driveView === 'collections' ? (
                    <CollectionsView
                        ...
                        onAnalyzeMissing={handleAnalyzeMissingAi}
                    />
                ) : driveView === 'timeline' ? (
                    <TimelineView
                        files={allFiles}
                    ...
                    <>
                        {activeFolderId === null && driveView === 'files' && searchTerm.length <= 2 && (
                            <AnalyticsBanner />
                        )}
                        <FileExplorer
...
function getReadableFolderPath(folderId: number | null, folders: TelegramFolder[]): string {
