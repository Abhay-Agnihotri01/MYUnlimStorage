...
import {
    ...
    telegramSetAutoIndexLegacy,
    telegramPurgeLegacyMessages,
    telegramSignIn,
    telegramSetFileTags,
    ...
    telegramUploadFile,
    telegramUpdateManifestSettings,
    telegramGetBandwidth,
...
async function invokeTauriCommand<T>(invoke: TauriInvokeFn, command: string, args: CommandArgs): Promise<T> {
    switch (command) {
        ...
        case 'cmd_set_auto_index_legacy':
        case 'cmd_purge_legacy_messages':
...
export async function getBrowserBlob(messageId: number): Promise<{ blob: Blob; name: string }> {
    if (isSavedMessagesDefaultStorage()) {
        const { telegramDownloadBlob } = await import('./telegramBrowser');
        return await telegramDownloadBlob(messageId);
    }
    ensureBrowserMode();
    const record = await getWebFile(messageId);
    if (!record || !record.blob) throw new Error('File not found');
    return { blob: record.blob, name: record.name };
}
...
async function invokeBrowserTelegramCommand<T>(command: string, args: CommandArgs): Promise<T> {
    switch (command) {
        ...
        case 'cmd_factory_reset':
            return await telegramFactoryReset() as T;
        case 'cmd_is_network_available':
            return navigator.onLine as T;
        case 'cmd_get_bandwidth':
            return await telegramGetBandwidth() as T;
        case 'cmd_get_files':
        ...
        case 'cmd_set_auto_index_legacy':
            return await telegramSetAutoIndexLegacy(Boolean(args.enabled)) as T;
        case 'cmd_purge_legacy_messages':
            return await telegramPurgeLegacyMessages() as T;
        case 'cmd_get_offline_cache_stats':
        ...
        case 'cmd_update_settings':
            return await telegramUpdateManifestSettings(args.settings as any) as T;
        default:
